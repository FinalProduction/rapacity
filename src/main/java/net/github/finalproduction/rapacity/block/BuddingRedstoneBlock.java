...
public class BuddingRedstoneBlock extends Block {
   ...
   /* public static final int GROWTH_CHANCE = 5;
    ...
    public void randomTick(BlockState state, ServerWorld level, BlockPos pos, Random random) {
        if (random.nextInt(5) == 0) {
            ...
            } else if (blockState.isOf(BlockRegistry.SMALL_REDSTONE_BUD.get()) && blockState.get(CrystalBlock.FACING) == direction) {
                block = BlockRegistry.MEDIUM_REDSTONE_BUD.get();
            } else if (blockState.isOf(BlockRegistry.MEDIUM_REDSTONE_BUD.get()) && blockState.get(CrystalBlock.FACING) == direction) {
                block = BlockRegistry.LARGE_REDSTONE_BUD.get();
            } else if (blockState.isOf(BlockRegistry.LARGE_REDSTONE_BUD.get()) && blockState.get(CrystalBlock.FACING) == direction) {
                block = BlockRegistry.REDSTONE_CLUSTER.get();
            }
